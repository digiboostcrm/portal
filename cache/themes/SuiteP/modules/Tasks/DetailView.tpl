...
{if !$fields.rightsignature_c.hidden}
{counter name="panelFieldCount" print=false}

{if !empty($fields.ut_rightsignature_id_c.value)}
{capture assign="detail_url"}index.php?module=UT_RightSignature&action=DetailView&record={$fields.ut_rightsignature_id_c.value}{/capture}
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="ut_rightsignature_id_c" class="sugar_field" data-id-value="{$fields.ut_rightsignature_id_c.value}">{$fields.rightsignature_c.value}</span>
{if !empty($fields.ut_rightsignature_id_c.value)}</a>{/if}
{/if}
