...
{counter name="panelFieldCount"  print=false}
<input tabindex="0"  name="account_name" id="EditView_account_name" {if ($fields.converted.value == 1)}disabled="true"{/if} size="30" maxlength="255" type="text" value="{$fields.account_name.value}">
</div>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_LEAD_SOURCE_1' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.lead_source_1_c.name}" 
id="{$fields.lead_source_1_c.name}" 
title=''       
>
{if isset($fields.lead_source_1_c.value) && $fields.lead_source_1_c.value != ''}
{html_options options=$fields.lead_source_1_c.options selected=$fields.lead_source_1_c.value}
{else}
{html_options options=$fields.lead_source_1_c.options selected=$fields.lead_source_1_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.lead_source_1_c.options }
{capture name="field_val"}{$fields.lead_source_1_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.lead_source_1_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.lead_source_1_c.name}" 
id="{$fields.lead_source_1_c.name}" 
title=''          
>
{if isset($fields.lead_source_1_c.value) && $fields.lead_source_1_c.value != ''}
...
<input
id="{$fields.lead_source_1_c.name}-input"
name="{$fields.lead_source_1_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.lead_source_1_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.lead_source_1_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.lead_source_1_c.name}-input', '{$fields.lead_source_1_c.name}');sync_{$fields.lead_source_1_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.lead_source_1_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.lead_source_1_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.lead_source_1_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.lead_source_1_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.lead_source_1_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.lead_source_1_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.lead_source_1_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_NEXT_STEP' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
maxlength='255' 
value='{$value}' title='Enter the next steps for this lead'      >
</div>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_LEAD_TYPE' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CONTACT_LEAD' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_TRACKING_NUMBER' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{if strlen($fields.tracking_number.value) <= 0}
{assign var="value" value=$fields.tracking_number.default_value }
{else}
{assign var="value" value=$fields.tracking_number.value }
{/if}  
<input type='text' name='{$fields.tracking_number.name}' 
id='{$fields.tracking_number.name}' size='30' 
value='{$value}' title=''      >
</div>
...
addToValidate('EditView', 'sdr', 'enum', true,'{/literal}{sugar_translate label='LBL_SDR' module='Leads' for_js=true}{literal}' );
...
addToValidate('EditView', 'contact_id_c', 'id', false,'{/literal}{sugar_translate label='LBL_CONTACT_LEAD_CONTACT_ID' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'lead_source_1_c', 'enum', true,'{/literal}{sugar_translate label='LBL_LEAD_SOURCE_1' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'tracking_number', 'varchar', false,'{/literal}{sugar_translate label='LBL_TRACKING_NUMBER' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'next_step_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_NEXT_STEP' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'contact_lead_c', 'relate', false,'{/literal}{sugar_translate label='LBL_CONTACT_LEAD' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'lead_type', 'enum', true,'{/literal}{sugar_translate label='LBL_LEAD_TYPE' module='Leads' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'contact_lead_c', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Leads' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CONTACT_LEAD' module='Leads' for_js=true}{literal}', 'contact_id_c' );
