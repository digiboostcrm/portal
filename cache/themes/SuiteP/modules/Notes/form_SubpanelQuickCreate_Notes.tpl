...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_NOTES_TYPE' module='Notes'}{/capture}
{$label|strip_semicolon}:
...
<select name="{$fields.notes_type.name}" 
id="{$fields.notes_type.name}" 
title=''       
>
{if isset($fields.notes_type.value) && $fields.notes_type.value != ''}
{html_options options=$fields.notes_type.options selected=$fields.notes_type.value}
{else}
{html_options options=$fields.notes_type.options selected=$fields.notes_type.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.notes_type.options }
{capture name="field_val"}{$fields.notes_type.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.notes_type.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.notes_type.name}" 
id="{$fields.notes_type.name}" 
title=''          
>
{if isset($fields.notes_type.value) && $fields.notes_type.value != ''}
...
<input
id="{$fields.notes_type.name}-input"
name="{$fields.notes_type.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.notes_type.name}-image"></button><button type="button"
id="btn-clear-{$fields.notes_type.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.notes_type.name}-input', '{$fields.notes_type.name}');sync_{$fields.notes_type.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.notes_type.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.notes_type.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.notes_type.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.notes_type.name}');
			...
			//global variable 
			sync_{/literal}{$fields.notes_type.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.notes_type.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.notes_type.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
addToValidate('form_SubpanelQuickCreate_Notes', 'opportunities_notes_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_OPPORTUNITIES_NOTES_1_FROM_OPPORTUNITIES_TITLE' module='Notes' for_js=true}{literal}' );
addToValidate('form_SubpanelQuickCreate_Notes', 'created_from_c', 'enum', true,'{/literal}{sugar_translate label='LBL_CREATED_FROM' module='Notes' for_js=true}{literal}' );
addToValidate('form_SubpanelQuickCreate_Notes', 'notes_type', 'enum', false,'{/literal}{sugar_translate label='LBL_NOTES_TYPE' module='Notes' for_js=true}{literal}' );
