...
{capture name="label" assign="label"}{sugar_translate label='LBL_PORTAL_USER' module='Documents'}{/capture}
{$label|strip_semicolon}:
</div>
...
{if !$fields.portal_user_c.hidden}
{counter name="panelFieldCount" print=false}
...
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="contact_id_c" class="sugar_field" data-id-value="{$fields.contact_id_c.value}">{$fields.portal_user_c.value}</span>
