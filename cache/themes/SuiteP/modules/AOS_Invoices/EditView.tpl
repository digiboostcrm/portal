...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_TYPE_C' module='AOS_Invoices'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.type_c.name}" 
id="{$fields.type_c.name}" 
title=''       
>
{if isset($fields.type_c.value) && $fields.type_c.value != ''}
{html_options options=$fields.type_c.options selected=$fields.type_c.value}
{else}
{html_options options=$fields.type_c.options selected=$fields.type_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.type_c.options }
{capture name="field_val"}{$fields.type_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.type_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.type_c.name}" 
id="{$fields.type_c.name}" 
title=''          
>
{if isset($fields.type_c.value) && $fields.type_c.value != ''}
...
<input
id="{$fields.type_c.name}-input"
name="{$fields.type_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.type_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.type_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.type_c.name}-input', '{$fields.type_c.name}');sync_{$fields.type_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.type_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.type_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.type_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.type_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.type_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.type_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.type_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CCUSTOM_FILE' module='AOS_Invoices'}{/capture}
{$label|strip_semicolon}:
...
addToValidate('EditView', 'qbid_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_QBID' module='AOS_Invoices' for_js=true}{literal}' );
addToValidate('EditView', 'type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TYPE_C' module='AOS_Invoices' for_js=true}{literal}' );
addToValidate('EditView', 'created_from_c', 'enum', true,'{/literal}{sugar_translate label='LBL_CREATED_FROM' module='AOS_Invoices' for_js=true}{literal}' );
addToValidate('EditView', 'custom_file', 'file', false,'{/literal}{sugar_translate label='LBL_CREDIT_CARD_MASTER12' module='AOS_Invoices' for_js=true}{literal}' );
