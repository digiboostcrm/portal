...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_USER_HASH' module='Users'}{/capture}
{$label|strip_semicolon}:
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_FACTOR_AUTH_INTERFACE' module='Users'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.factor_auth_interface.name}" 
id="{$fields.factor_auth_interface.name}" 
title=''       
>
{if isset($fields.factor_auth_interface.value) && $fields.factor_auth_interface.value != ''}
{html_options options=$fields.factor_auth_interface.options selected=$fields.factor_auth_interface.value}
{else}
{html_options options=$fields.factor_auth_interface.options selected=$fields.factor_auth_interface.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.factor_auth_interface.options }
{capture name="field_val"}{$fields.factor_auth_interface.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.factor_auth_interface.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.factor_auth_interface.name}" 
id="{$fields.factor_auth_interface.name}" 
title=''          
>
{if isset($fields.factor_auth_interface.value) && $fields.factor_auth_interface.value != ''}
...
<input
id="{$fields.factor_auth_interface.name}-input"
name="{$fields.factor_auth_interface.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.factor_auth_interface.name}-image"></button><button type="button"
id="btn-clear-{$fields.factor_auth_interface.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.factor_auth_interface.name}-input', '{$fields.factor_auth_interface.name}');sync_{$fields.factor_auth_interface.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.factor_auth_interface.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.factor_auth_interface.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.factor_auth_interface.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.factor_auth_interface.name}');
			...
			//global variable 
			sync_{/literal}{$fields.factor_auth_interface.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.factor_auth_interface.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.factor_auth_interface.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.messenger_type.name}-input', '{$fields.messenger_type.name}');sync_{$fields.messenger_type.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
addToValidate('EditView', 'custom_user_type', 'varchar', false,'{/literal}{sugar_translate label='LBL_USER_TYPE' module='Users' for_js=true}{literal}' );
