...
function caseUpdates(record){
    ...
    var update_data = encodeURIComponent(document.getElementById('update_text_1').value);
    var checkbox = document.getElementById('internal_1').checked;
    var internal = "";
    if(checkbox){
        internal=1;
    }
...
function quick_edit_case_updates($case)
{
    ...
    <div><label for="update_text_1">{$mod_strings['LBL_UPDATE_TEXT']}</label></div>
    <textarea id="update_text_1" name="update_text" cols="80" rows="4"></textarea>

    <div><label>{$mod_strings['LBL_INTERNAL']}</label>
    <input id='internal_1' type='checkbox' name='internal' tabindex=0 title='' value='1' $internalChecked ></input>
    </div>
